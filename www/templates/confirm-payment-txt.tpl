English follows Dutch
--------------------------------------------------------------------------------

Geachte {$registration->buyer()->name|escape:'htmlall'},

Je betaling van € {$registration->sprice()} is in goede orde ontvangen en je reservering staat hierbij vast. 

We wensen je veel plezier tijdens het Arabian Nights Gala!
        
Dansende groeten,
Andriy, Helena, Marjolein en Tom
GalaCie Commissie 2016 - 2017
SDV AmsterDance

--------------------------------------------------------------------------------

Dear {$registration->buyer()->name|escape:'htmlall'}, 

Your payment of € {$registration->sprice()} has been fully received and your reservation has been fully confirmed.

We wish you a magical evening at the Arabian Nights Gala!

Kind regards,
Andriy, Helena, Marjolein and Tom
GalaCie Commissie 2016 - 2017
SDV AmsterDance

--------------------------------------------------------------------------------

...
function checkForm() {
  ...
  name = $("#buyer-name").val();
  if (name.length == 0) { $("#buyer-name").addClass("error"); return false; } else $("#buyer-name").removeClass("error");
  data["name"] = name;
  ...
  for (i = 0; i < guests.length; i++) {
    ...
    //Extract name, checking length and store in data. 
    if (name.length == 0) { $(guests[i]).find(".buyer-name").addClass("error"); return false; } else $(guests[i]).find(".buyer-name").removeClass("error");
    data["guests"].push({ name: name, member: (member == "1"), vip: (vip == "2") });
  }

  //Extract email, checking length and store in data. 
  email = $("#buyer-email").val();
  if (email.length == 0 || !validEmail(email)) { $("#buyer-email").addClass("error"); return false; } else $("#buyer-email").removeClass("error");
  data["email"] = email;
  ...
  $.post('api/register/', { data: JSON.stringify(data) }, function(data) {
    console.log(data);
  });
...
function fixStatus() {
  ...
  jQuery.getJSON('api/status/', function(data) {
    // set the inner value
